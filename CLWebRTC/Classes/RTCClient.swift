...
//        let constraints = [kRTCMediaConstraintsMinAspectRatio:"2.0"]// kRTCMediaConstraintsMaxWidth : "360", kRTCMediaConstraintsMaxHeight: "640",
...
//        print("didAdd stream \(stream.videoTracks.)")
    ...
    public func peerConnection(_ peerConnection: RTCPeerConnection, didRemove stream: RTCMediaStream) {
        ...
        print("didRemove stream \(stream.audioTracks.count)")
        if stream.videoTracks.count > 0 ,let id = getPeerId(peerConnection){
            self.delegate?.rtcClient(id,client: self, didRemoveRemoteVideoTrack: stream.videoTracks[0])
        }
    }
